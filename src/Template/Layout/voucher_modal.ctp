...
<script>
    var urlredrict = "<?php echo $this->Url->build(['controller' => 'Devices', 'action' => 'viewQcVoucher' . '/' . UrlUtil::_encodeUrl($infor_devices->id)])?>";
</script>
<?php
if (isset($infor_devices->type) && $infor_devices->type == 1) {
    echo $this->element('Landing/voucher_tplink');
} else {
    echo $this->element('Landing/voucher_mikrotic');
}
?>
